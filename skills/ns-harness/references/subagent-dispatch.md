...
When skill must **spawn separate agent** (Cursor/Claude project agent, Task tool, `/name`): **MUST** use harness **thin bridge** if available. Bare skill call / invented persona = forbidden while bridge exists.

Skills = **workflow source of truth**. Bridges bind `model` / `readonly`, load `AGENTS.md` + skill.
...
3. Pass task context (paths, `ISSUE_URL`, unit scope, mode) in dispatch message; bridge still opens `AGENTS.md` then skill.
4. **In-session exception:** only when user already invoked **that same** skill/bridge as session face (e.g. `/ns-code-coder`, `/task-writer-agent`). Continue that face — do not bounce to bridge. Does **not** apply when parent is `ns-spec-driven` (or handoff/orchestrator/autonomous/gitlab) and phase is a **child** in the table above.
...
`reviewer-agent` = required review-gate vehicle when present — loads `ns-code-reviewer`; not a substitute for that skill.
