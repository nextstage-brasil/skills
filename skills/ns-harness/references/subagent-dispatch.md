...
6. Platform cannot spawn `{name}` (only `inherit` / `coder` / `reviewer` / `generalPurpose` / other persona): **stop**. Tell human invoke `/{name}`. Do **not** spawn inherit-as-bridge.
...
| | `coder` / `reviewer` / `generalPurpose` / `explore` as stand-in for a named bridge |
