...
Presets get bridges on `init` / `sync` / `update`. Bridge body: obey `AGENTS.md` (no tool-Read) → Session boot (`session-boot.md`) → skill. Vehicle `{name}` optional (binds `model`); mapped skill **required** — `subagent-dispatch.md`.
