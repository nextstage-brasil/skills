...
Project harness face. Other catalog skills still point here for **session boot** and artifact paths. This skill **also** runs brownfield onboarding via local references — no separate prepare/architecture/agents/reverse/brownfield skills.

**Consumer skill convention:** point to `references/session-boot.md` and require **Session boot (blocking)** before other steps. Do not duplicate the boot sequence in skill bodies or frontmatter `description`.

## Dispatch (this skill as face)

**Session boot** (`references/session-boot.md`) first — then pick **one** row. First matching signal wins.

| Signal | Reference |
| ------ | --------- |
| Prepare this repo / harness prepare / onboard brownfield / full prepare / stale context after refactor | `references/prepare.md` (runs the four workers in order) |
...
| `/ns-harness` with no qualifier | Show this table; ask once if intent is unclear |

Bare `/ns-harness prepare this repo` **is** full `prepare.md`. Do **not** send the user to retired slash names.

Read the chosen reference **in full** and follow it. Do not improvise a parallel workflow.
...
| `references/code-skill-routing.md` | Entry priority and handoffs between implementation skills |
...
Read only the file(s) the active workflow names. Do not assume stack unless detected in rules or context.
