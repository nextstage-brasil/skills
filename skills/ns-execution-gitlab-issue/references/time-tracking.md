...
| ------------ | ----------- |
| `START_TIME` | **External mode:** immediately before first coding invoke (`coder-agent` single-unit or `ns-autonomous` Engine). **SDD unit mode:** immediately before first task in unit (`run-implementation`). Not during gates, branch resolution, or worktree setup. |
