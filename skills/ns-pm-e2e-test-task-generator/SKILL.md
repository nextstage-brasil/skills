---
name: ns-pm-e2e-test-task-generator
description: (NS) Generate E2E test task markdown for Cypress/TypeScript during SDD planning after frontend tasks exist. Use when planning Cypress coverage per feature — not for writing specs (use ns-code-e2e-tests). Requires data-testid contract from frontend task. Device-aware desktop/tablet/mobile structure.
license: Apache-2.0
metadata:
...
Planning-phase skill: produce E2E task markdown. Execution-phase: `ns-code-e2e-tests`.

## Boundary

| Phase                         | Skill                |
| ----------------------------- | -------------------- |
| Planning (task file)          | `ns-pm-e2e-test-task-generator` |
| Implementation (Cypress code) | `ns-code-e2e-tests`   |
...
- Bootstrap note when project has no E2E yet: executor must run `ns-code-e2e-tests` Phase 0 before specs
...
- `ns-code-e2e-tests` — write/refactor Cypress code
- `ns-sdd-task-generator` — frontend tasks with testid contract
