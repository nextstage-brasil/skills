---
name: ns-code-backend-tests
description: (NS) Create and maintain PHPUnit unit and integration tests in the backend package (bootstrap tests/ when missing). Mandatory Docker-only execution with phpunit --testdox --stop-on-failure --stop-on-error and 120s timeout. Use when writing or refactoring PHPUnit tests, implementing unit-test tasks, or scaffolding backend tests on greenfield Laravel/PHP projects — not when planning test task markdown (use ns-pm-unit-test-task-generator). Read harness backend-tests rules when present.
license: Apache-2.0
metadata:
...
Execution-phase PHPUnit implementation. Planning-phase: `ns-pm-unit-test-task-generator`.
...
4. **Task contract** — when implementing a `ns-pm-unit-test-task-generator` task, follow its scenario checklist
...
- Hung/timeout: state 120s limit exceeded; invoke `ns-code-investigator` if root cause unclear
...
| `ns-pm-unit-test-task-generator` | Task contract source |

## Related skills

- `ns-pm-unit-test-task-generator` — planning tasks
- `ns-code-investigator` — failing or hung test debugging
- `ns-code-coder` — ad-hoc fixes outside planned unit-test tasks
