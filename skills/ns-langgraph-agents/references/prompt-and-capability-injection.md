...
Mode-resolved `injected` (per-turn body + schema table): `references/prompt-and-capability-injection-plan-execute.md`.
...
## plan_execute / plan shape

When locking Prompt/Capability plan or `plan_execute` gather vs composer: read `references/prompt-and-capability-injection-plan-execute.md`.
