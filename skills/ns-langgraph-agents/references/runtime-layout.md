...
│   │   ├── guards.ts
│   │   └── nodes/*.node.ts # one file per node
...
│   ├── skills/             # registry, loader, to-langchain-tool
...
| Qualify/conversation prompts in `llm/` | `conversation/` + schemas |
| `llm/` imports `graph/` | Nodes orchestrate |
| `fixtures/`, `qualify/`, `rag/` as orphan folders | `config/`, `conversation/`, `catalog/` |
| `src/**/*.test.ts` | `tests/**` |
| Monolithic `nodes/index.ts` with all logic | One `*.node.ts` per node; index re-exports |
...
4. Context window module
5. Capability layer + local tools
6. MCP client + governance
7. Agent node with trim + tool loop
...
Pass criteria: no tests under `src/`; graph in `graph/`; `npm test` uses `CHECKPOINTER=memory` and `LLM_DISABLED=true`.
