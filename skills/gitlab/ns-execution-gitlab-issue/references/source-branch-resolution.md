...
Used by Gate 1 in `ns-execution-gitlab-issue` (skipped when `REUSE_MODE = true`).
...
| 3 | **Product rule** | `.nextstage-harness/rules/branch-resolution.md` when present. |
...
When `.nextstage-harness/rules/branch-resolution.md` exists, apply it at priority 3 **before** milestone/version discovery. Its output is still subject to remote validation and separator alternates above.
