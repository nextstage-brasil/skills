# harness-codebase-reverse-spec v3

Evolution of `.cursor/skills/harness-codebase-reverse-spec/`. The original skill **delivers the expected outcome** — this v3 tightens the process where gaps caused rework, inconsistency, or portability issues.
...
```
.cursor/skills/harness-codebase-reverse-spec/     → original (unchanged)
.cursor/skills/harness-codebase-reverse-spec-v3/   → this delivery
```

To adopt v3: copy `harness-codebase-reverse-spec-v3/` to `harness-codebase-reverse-spec/` or update the skill path in your agent config.
