...
- Completes `ns-coder`'s full per-task cycle including the **review gate** (`../../../code/ns-reviewer/references/review-gate-workflow.md`) — **`reviewer-agent` / `ns-reviewer` only**; report unit `blocked` if review rounds exhaust without `Approved`.
