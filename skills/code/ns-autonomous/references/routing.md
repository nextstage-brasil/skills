...
- **Do not** redirect to `ns-execution-gitlab-issue` from this skill or from `ns-coder` subagents — `G` is already the lifecycle owner.
...
- Call any GitLab-mutating MCP tool (status labels, comments, MR creation, spent time). Those belong to `ns-execution-gitlab-issue`.
...
- The human asks for autonomous multi-step implementation without pointing at an existing SDD version's `execution-handoff.md` (that case belongs to `ns-spec-driven` → `references/orchestrator.md` instead — redirect if you detect it).

In standalone mode this skill is the only owner of the run: no `ns-execution-gitlab-issue` phases apply, and GitLab is out of scope unless `docs/context/gitlab-sync-config.md` exists and the human explicitly asks for an MR at closure.
