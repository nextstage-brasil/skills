...
Canonical review gate: `../../code/ns-reviewer/references/review-gate-workflow.md` — steps 7–9 **ad-hoc / C2 only**. Review **only** via `reviewer-agent` then `ns-reviewer` (**MUST** bridge when available; else skill direct); max **3** rounds; Critical fix requires **re-review** mandatory; no success without pass or **blocked**. After **Approved**: **Living specs** (8) if match, then **Final report** (9).
...
7. **Review loop** — **MUST** `reviewer-agent` when available (else `ns-reviewer`); `../../code/ns-reviewer/references/review-gate-workflow.md`
...
After step 6, run `../../code/ns-reviewer/references/review-gate-workflow.md` before done — **except SDD handoff** (return to parent; no review).
