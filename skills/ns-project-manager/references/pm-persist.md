...
Before first write/overwrite for a given basename, **look for the same filename outside** `pm/` under `docs/versions/{version_san}/` (typical: version root). Also detect **both** a root copy and a `pm/` copy.
...
4. If **both** root and `pm/` exist: propose keep `pm/`, update refs that still point at root, then remove the root copy after refs are clean. Do **not** merge contents.
...
- Create `pm/` if needed.
- Move (or, if both exist: keep `pm/`, then remove the root copy after refs are clean). Do not reset Sequência on the move itself.
