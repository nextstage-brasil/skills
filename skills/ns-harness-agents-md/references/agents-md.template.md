...
**Before implementation, read `{harness_root}/rules/architecture-rules.md`** when it exists. If missing, run `ns-harness-architecture-rules` first.
...
Invoke via the Skills menu / slash (e.g. `/ns-code-coder`, `/ns-code-reviewer`, `/ns-execution-orchestrator`). **Skills are the entry points** — do not invent or document a separate "Agent personas / subagents" section.
