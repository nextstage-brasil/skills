...
- **Execution:** implement tests per task — use `code-backend-tests` or implementation workflow
...
- **Run command** — `vendor/bin/phpunit --testdox --stop-on-failure --stop-on-error` in Docker with 120s timeout (see `code-backend-tests`)
- Bootstrap note when project has no `tests/` yet: executor must run `code-backend-tests` Phase 0 before test classes
...
- `pm-task-generator` — implementation tasks first
- `code-backend-tests` — execution and bootstrap
- `pm-e2e-test-generator` — UI coverage
