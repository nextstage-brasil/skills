...
Pre-planning workflow: identify and resolve ambiguities **before** invoking `ns-sdd-requirements-generator`.
...
3. **If brownfield signals exist and `brownfield-map.md` is missing:**
   ...
   - Invoke **`ns-harness-bootstrap-brownfield`** and produce `{product_root}/docs/context/brownfield-map.md`.
...
6. On **`refresh`:** invoke **`ns-harness-bootstrap-brownfield`** (update in place), then continue to Step 1 with the new map.
...
### Step 4 — Hand off to ns-sdd-requirements-generator
...
Pass this document as input to `ns-sdd-requirements-generator`. Do **not** write `requirements.md` here.
...
```
planning start → [brownfield + no map?] → ns-harness-bootstrap-brownfield → [brownfield refresh gate] → ns-sdd-clarify-requirements → ns-sdd-requirements-generator → Gate 1
```
...
- `ns-harness-bootstrap-brownfield` — mandatory prerequisite when code exists and `brownfield-map.md` is missing
- `ns-sdd-requirements-generator` — next step after clarification
- `ns-harness-architecture-rules` — constitution when `architecture-rules.md` is still a stub (separate from this skill)
