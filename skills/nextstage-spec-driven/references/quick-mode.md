...
4. Delegate to **`code-coder`** with explicit `{task_description}`.
5. Optional: user asks for review → `code-reviewer` (not automatic).

## Do not

- Create `requirements.md` or task files.
- Run clarify / specify / task-generator.
- Auto-run `harness-prepare`.
...
→ **Stop**, announce upgrade to Medium pipeline, delegate to `pm-requirements-generator` and downstream workers.
