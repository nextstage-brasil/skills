...
3. **Create its own worktree**: `{product_root}/.worktrees/{version_san}/` + branch `work/{version_san}` from the resolved base branch, following `../nextstage-harness/references/worktree-setup.md`. Abort on conflict — see `references/standalone-pipeline.md`.
