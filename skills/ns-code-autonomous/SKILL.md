...
5. **Internal review loop** — `../ns-harness/references/review-gate-workflow.md`: **MUST** invoke **`reviewer-agent`** when available (else **`ns-code-reviewer`**) (version-closure mode) only; max 3 rounds; `Approved` requires score ≥9/10 (ideal 10); fix units on `Rejected` (Criticals or score ≤8), then **mandatory re-review**; stop on `Blocked` or rounds exhausted.
...
| `ns-code-coder`           | Subagent implementation via `coder-agent` (**MUST** when available) |
| `ns-code-reviewer`        | Review gate via `reviewer-agent` (**MUST** when available) |
