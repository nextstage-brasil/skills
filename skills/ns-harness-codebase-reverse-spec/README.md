# ns-harness-codebase-reverse-spec v3

Evolution of `.cursor/skills/ns-harness-codebase-reverse-spec/`. The original skill **delivers the expected outcome** — this v3 tightens the process where gaps caused rework, inconsistency, or portability issues.
...
Rewritten to be more inclusive of trigger phrases (per ns-skill-creator guidance on under-triggering), without changing the skill's scope exclusions.
...
```
.cursor/skills/ns-harness-codebase-reverse-spec/     → original (unchanged)
.cursor/skills/ns-harness-codebase-reverse-spec-v3/   → this delivery
```

To adopt v3: copy `ns-harness-codebase-reverse-spec-v3/` to `ns-harness-codebase-reverse-spec/` or update the skill path in your agent config.
