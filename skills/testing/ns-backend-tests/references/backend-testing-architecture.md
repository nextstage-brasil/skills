...
| Scenario | Root | Rule |
| -------- | ---- | ---- |
| **Monorepo** | `backend/` | PHPUnit lives inside the backend package |
| **Standalone API** | `.` | When `phpunit.xml` is at repo root |
| **Brownfield** | Detected path | Use as-is; do not relocate `tests/` |

### Detection order

Check under the repo root (and `backend/` when present):
...
| Hung run | Report blocker; suggest `ns-investigator` if cause unclear |
