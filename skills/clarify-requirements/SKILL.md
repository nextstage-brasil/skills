...
3. **If brownfield signals exist and `brownfield-map.md` is missing:**
   ...
   - Invoke **`bootstrap-brownfield`** and produce `{product_root}/docs/context/brownfield-map.md`.
...
(re-scan codebase with bootstrap-brownfield) or keep the existing map?
...
6. On **`refresh`:** invoke **`bootstrap-brownfield`** (update in place), then continue to Step 1 with the new map.
...
- **Brownfield without map:** run `bootstrap-brownfield` first — never skip by grepping the codebase instead
...
```
planning start → [brownfield + no map?] → bootstrap-brownfield → [brownfield refresh gate] → clarify-requirements → requirements-generator → Gate 1
```
...
- `bootstrap-brownfield` — mandatory prerequisite when code exists and `brownfield-map.md` is missing
- `requirements-generator` — next step after clarification
- `architecture-rules-generator` — constitution when `architecture-rules.md` is still a stub (separate from this skill)
