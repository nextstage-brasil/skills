...
Canonical review gate: `../ns-harness/references/review-gate-workflow.md` — follow exactly for steps 7–9. Review **only** via `ns-code-reviewer`; max **3** rounds; after any Critical fix, **re-review** is mandatory; never report success without a passing verdict or an explicit **blocked** state. After **Approved**, run **Living specs** (step 8) when conditions match, then **Final report** (step 9).
