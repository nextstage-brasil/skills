...
Proceed straight to dispatch (`multi-agent-dispatch.md`) with a single implicit unit. Do not create `requirements.md`, `tasks/`, or `execution-plan.md` for this path — that would be planning overhead for work that doesn't need it (same principle `ns-coder` follows for ad-hoc tasks).
