...
const SDD_PLANNING = [
  ...
  'pm-unit-test-task-generator',
  'pm-e2e-test-generator',
];
...
function buildImplementationNote(installed) {
  const set = new Set(installed);
  if (set.has('execution-gitlab-issue')) {
    return 'GitLab issue → `execution-gitlab-issue` (delegates coding to `code-autonomous`); local plan/ad-hoc autonomous → `code-autonomous` standalone.';
  }
...
NextStage harness project. Skills detected from \`.agents/skills/\` (generated by CLI — refine with \`harness-agents-md\` skill for brownfield).
