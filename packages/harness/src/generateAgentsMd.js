...
function buildSubagentsSection(projectRoot, installed) {
  ...
  return `Invoke via Cursor/Claude project agents (e.g. \`/coder-agent\`, \`/reviewer-agent\`, \`/task-writer-agent\`) — **exact** \`name\` from this table when spawn gate requires it (\`subagent-dispatch.md\`). Orchestrators **MUST** spawn named bridge for heavy execute / Tasks / Review so YAML \`model:\` applies. Cheap execute (Quick / host P5) = in-session \`ns-coder\` — **MUST NOT** spawn \`coder-agent\`. **FORBIDDEN:** child Task \`inherit\` / \`coder\` / \`reviewer\` / \`generalPurpose\` as stand-in (parent model leaks). Inline mapped skill while bridge present **and** gate requires spawn = forbidden. Each bridge obeys this \`AGENTS.md\` (already in context — no tool-Read), boots rules per \`session-boot.md\`, then the mapped skill. Edit \`model\` in \`.nextstage-harness/manifest.json\` → \`subagents\`; \`harness update\` never resets your model.
