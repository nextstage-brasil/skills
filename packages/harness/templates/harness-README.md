...
Orchestrators **MUST** spawn the adapter by **exact** `name` (`.cursor/agents/{name}.md` / `/{name}`) so that YAML `model` applies. Cursor Task types `coder` and `reviewer` are **not** these bridges. Spawning Task with `inherit` (or `coder` / `reviewer` / `generalPurpose`) uses the **parent** model / platform persona — forbidden while the bridge exists (`ns-harness` → `references/subagent-dispatch.md`). If the platform cannot bind `{name}`, stop and ask the human to invoke `/{name}`.
