...
Orchestrators follow spawn gate in `ns-harness` → `references/subagent-dispatch.md`: **MUST** spawn adapter by **exact** `name` for heavy execute / Tasks / Review so YAML `model` applies; cheap execute = in-session `ns-coder` (**MUST NOT** spawn `coder-agent`). Cursor Task types `coder` and `reviewer` are **not** these bridges. Spawning Task with `inherit` (or `coder` / `reviewer` / `generalPurpose`) uses the **parent** model / platform persona — forbidden when gate requires the named bridge. If the platform cannot bind `{name}`, stop and ask the human to invoke `/{name}`.
